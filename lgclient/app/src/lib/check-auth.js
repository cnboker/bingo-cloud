...
// export function refreshClient(client) {     const storedToken =
// localStorage.getItem("token");     // if it exists     if (storedToken) { //
// parse it down into an object         const token = JSON.parse(storedToken);
//       var jsTicks = new Date().getTime() * 10000 + 621355968000000000;
//  // if the token has expired return false console.log("span time",
// (token.expired - jsTicks) / 10000);         // if (token.expired > jsTicks) {
//         client = {             // ...client,        ...token         };
// // }     } }
